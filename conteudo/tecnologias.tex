...
necessidade destas arquiteturas se adaptarem e evoluirem para suprir estes
novos requisitos.
...
a aplicação como um todo, ao contrário de sistemas implementados com
\ac{SOA}, onde os serviços devem continuar a disponibilizar suas
funcionalidades, na medida do possível, mesmo que um serviço o qual este
depende apresente algum tipo de problema.
...
reflete em consumo de recursos e tempo de computação, que podem ser evitados
caso não seja mantido estados. Para tal, a realização de cada requisição na
comunicação deve carregar todas as informações necessárias para realizaçao da
operação. Este conceito de comunicação é aplicado em diversos protocolos de
comunicação assincronos, como o \ac{HTTP}, os quais, por limitações físicas,
...
os outros serviços estão rodando e como acessá-los. Isto pode ser feito por
...
a composição das regras de negócio por diversos serviços.
...
realizadas as entregas de novas versões, no gerenciamento dos ambientes
...
que disponibilizam imagens prontas para diversos serviços e tecnologias.
Também existe a possibilidade de ser criado um repositório privado, caso
alguma política de controle de licenças não permita a criação de repositórios
públicos.
...
para rodar estas aplicações. Isto é feito por meio da utilização de
...
ser também a nível de processos. Segundo \citeonline{Matthias2015},
