...
Microserviços surgiram de práticas aplicadas por empresas de escala global que
operam na internet, muito antes de haver um nome para estas práticas. Estas
práticas se enquadram como um subconjunto dos conceitos de \ac{SOA}, mas se
diferencia em alguns pontos das tecnologias tradicionais por enforçar práticas
...
sejam escritos em linguagens diferentes, caso seja necessário. Isto pode
ser útil caso seja necessário utilizar tecnologias diferentes em um
...
\ac{REST} é um padrão de criação de \ac{API} que vem ganhando mercado com a
adoção em massa por grandes empresas da internet. Ele define algumas práticas
para estruturação das \ac{URL} por meio da utilização do que ele chama de
...
engloba. Alguns exemplos de recursos são usuãrios, \emph{tickets} de um sistema
de controle de bugs e notas fiscais de um software de \ac{ERP}.
...
por \emph{sub-URLs} para ações mais específicas. O conjunto de ações mais
comuns é \ac{CRUD}, as quais são operações básicas de manipulação de dados,
que são traduzidos para os métodos \emph{POST}, \emph{GET}, \emph{PUT} e
\emph{DELETE} para utilização na \ac{API}.

Como apresentado por \citeonline{Ignacio2009}, o padrão \ac{REST} funciona
muito bem em conjunto com microserviços, por aplicar algumas práticas de
separação de funcionalidades por meio de isolamento de recursos relacionados.
No estudo de caso deste artigo, é apresentado a criação de uma \ac{API}
\ac{REST} de pesquisa, na qual o resurso apresentado é o objeto alvo
da pesquisa, o qual é repassado para microserviços que funcionam como
intermediários para busca em motores de busca disponíveis no mercado.
