...
síncrona e assíncrona entre serviços, mas gerou uma complicação, pois os serviços
não exportavam mais \acp{API} \ac{HTTP}. Para resolver esta situação, a primeira
tentativa foi adicionar serviços de tradução das mensagens \ac{AMQP} de borda para
\ac{HTTP}, o que vai contra o principal objetivo do projeto, que é facilitar
o desenvolvimento de novas funcionalidades e a manutenção do mesmo, pois seria
necessário todos os serviços vir em pares, aumentando o consumo de recursos.
Para resolver isto, foi adicionado um serviço único de tradução de protocolos,
o \emph{API Gateway}.
